...
Sets up the tx enable pin
Call once class has been instantiated, typically within setup().

@ingroup setup
*/
void ModbusMaster::SetTxEnablePin(uint8_t TxEnablePin)
{
...
void ModbusMaster::SlaveAddress(uint8_t slave)
{
  _u8MBSlave = slave;
}
...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  if (_u16BaudRate == 19200) delayMicroseconds(250);
  if (_u16BaudRate == 9600) delayMicroseconds(500);
  if (_u16BaudRate == 4800) delayMicroseconds(1000);
  for (i = 0; i < u8ModbusADUSize; i++)
  {
  ...
  if (_u16BaudRate == 19200) delayMicroseconds(500);
  if (_u16BaudRate == 9600) delayMicroseconds(1000);
  if (_u16BaudRate == 4800) delayMicroseconds(2000);
